			...
			if(guildData.plugins.goodbye.enabled){
				...
				if(channel){
					...
					if(guildData.plugins.goodbye.withImage){
						...
						ctx.lineWidth = 15;
						ctx.strokeText(member.guild.translate("administration/goodbye:TITLE"), canvas.width - 430, canvas.height - 320);
						...
						ctx.fillText(member.guild.translate("administration/goodbye:TITLE"), canvas.width - 430, canvas.height - 320);
						// Draw username
						ctx.fillStyle = "WHITE";
						ctx.shadowColor = "BLACK";
						ctx.shadowBlur = 10;
