...
class Userinfo extends Command {
	...
	async run (message, args, data) {
		...
		console.log(roles.slice(0, 14));
			...
			`**● ${message.translate("common:ROLES")} [${roles.length}]:** ${roles.length < 16 && roles.length !== 0 ? roles.join(", ") : roles.length > 15 ? roles.join(", ") `\`${message.translate("common:AND_MORE")}\`` : `\`${message.translate("common:NONE")}\``}`,
