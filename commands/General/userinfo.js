...
class Userinfo extends Command {
	...
	async run (message, args, data) {
		...
		.map(role => role.toString());
			...
			`**● ${message.translate("common:ROLES")} [${roles.length}]:** ${roles.length < 16 && roles.length !== 0 ? roles.slice(0, -1).join(', ') : roles.length > 15 ? roles.slice(0, 14).join(", ") `\`${message.translate("common:AND_MORE")}\`` : `\`${message.translate("common:NONE")}\``}`,
