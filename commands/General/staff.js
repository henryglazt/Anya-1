...
class Staff extends Command {
    ...
    async run(message, args, data) {
        ...
        const embed = new MessageEmbed()
            ...
            .addField(message.translate("general/staff:ADMINS"), administrators.size > 0 ? administrators.map((a) => `${this.client.customEmojis.status[a.presence.status]} | ${escapeMarkdown(a.user.tag)}`)
                    .join("\n") : message.translate("general/staff:NO_ADMINS"))
            .addField(message.translate("general/staff:MODS"), moderators.size > 0 ? moderators.map((m) => `${this.client.customEmojis.status[m.presence.status]} | ${escapeMarkdown(m.user.tag)}`)
